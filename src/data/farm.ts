export type FarmSection = 'Inventory' | 'Equipment' | 'Finance' | 'Reports';

export type StockStatus = 'Available' | 'Low Stock' | 'Out of Stock';

export type EquipmentStatus = 'Available' | 'In Use' | 'Service';

export type InventoryItem = {
  id: string;
  name: string;
  category: string;
  status: StockStatus;
  quantity: number;
  quantityEtiqueta: string;
  minCantidad: number;
  minEtiqueta: string;
  unit: string;
  unitCosto: number;
  unitCostEtiqueta: string;
  priceEtiqueta: string;
  totalValueEtiqueta: string;
  supplier: string;
  purchaseFecha: string;
  progress: number;
  alert?: string;
  movements: StockMovement[];
};

export type StockMovement = {
  id: string;
  title: string;
  date: string;
  amountEtiqueta: string;
  kind: 'in' | 'out' | 'balance';
};

export type FarmEquipment = {
  id: string;
  name: string;
  type: string;
  modelAnio: string;
  icon: string;
  status: EquipmentStatus;
  hours: number;
  hoursEtiqueta: string;
  serviceIntervalHrs: number;
  hoursToServicio: number | null;
  serviceAlerta?: string;
  assignedCampo?: string;
  fuelTipo: string;
  maintCostEtiqueta: string;
  serviceProgreso: number;
};

export type ExpenseCategory = {
  id: string;
  label: string;
  amountEtiqueta: string;
  progress: number;
  color: string;
};

export type FarmTransaction = {
  id: string;
  title: string;
  subtitle: string;
  amountEtiqueta: string;
  kind: 'income' | 'expense';
  icon: string;
};

export type SeasonReport = {
  id: string;
  title: string;
  subtitle: string;
  metrics: {label: string; value: string; tone?: 'success' | 'danger' | 'gold'}[];
};

export const FARM_SECTIONS: FarmSection[] = [
  'Inventory',
  'Equipment',
  'Finance',
  'Reports',
];

export const FARM_DEMO_BANDA =
  'The current information is sample data. Once you add your own data, the app will use it across all relevant sections';

export const INVENTORY_ITEMS: InventoryItem[] = [
  {
    id: 'inv-corn',
    name: 'Pioneer P9241 Corn Grain',
    category: 'Sowing',
    status: 'Available',
    quantity: 18,
    quantityEtiqueta: '18 bags',
    minCantidad: 8,
    minEtiqueta: 'min 8 bags',
    unit: 'bags',
    unitCosto: 210,
    unitCostEtiqueta: '$210/bag',
    priceEtiqueta: '$210/bag',
    totalValueEtiqueta: '$3,780',
    supplier: 'Pioneer Co.',
    purchaseFecha: 'Mar 28, 2026',
    progress: 1,
    movements: [
      {
        id: 'm1',
        title: 'Stock Added',
        date: 'Mar 28',
        amountEtiqueta: '+20 bags',
        kind: 'in',
      },
      {
        id: 'm2',
        title: 'Used — North Field',
        date: 'Apr 18',
        amountEtiqueta: '−2 bags',
        kind: 'out',
      },
      {
        id: 'm3',
        title: 'Current Balance',
        date: '',
        amountEtiqueta: '18 bags',
        kind: 'balance',
      },
    ],
  },
  {
    id: 'inv-npk',
    name: 'NPK 16-16-16',
    category: 'Fertilizers',
    status: 'Low Stock',
    quantity: 240,
    quantityEtiqueta: '240 kg',
    minCantidad: 300,
    minEtiqueta: 'min 300 kg',
    unit: 'kg',
    unitCosto: 1.1,
    unitCostEtiqueta: '$1.10/kg',
    priceEtiqueta: '$1.10/kg',
    totalValueEtiqueta: '$264',
    supplier: 'AgriSupply Co.',
    purchaseFecha: 'Mar 12, 2026',
    progress: 0.8,
    movements: [
      {
        id: 'm1',
        title: 'Used — North Field',
        date: 'Jul 22',
        amountEtiqueta: '−4 kg',
        kind: 'out',
      },
      {
        id: 'm2',
        title: 'Stock Added',
        date: 'Jun 30',
        amountEtiqueta: '+50 kg',
        kind: 'in',
      },
      {
        id: 'm3',
        title: 'Used — River Plot',
        date: 'Jun 12',
        amountEtiqueta: '−8 kg',
        kind: 'out',
      },
      {
        id: 'm4',
        title: 'Used — East Field',
        date: 'May 14',
        amountEtiqueta: '−180 kg',
        kind: 'out',
      },
      {
        id: 'm5',
        title: 'Current Balance',
        date: '',
        amountEtiqueta: '240 kg',
        kind: 'balance',
      },
    ],
  },
  {
    id: 'inv-glyph',
    name: 'Glyphosate 360',
    category: 'Crop Protection',
    status: 'Out of Stock',
    quantity: 0,
    quantityEtiqueta: '0 L',
    minCantidad: 20,
    minEtiqueta: 'min 20 L',
    unit: 'L',
    unitCosto: 9.6,
    unitCostEtiqueta: '$9.60/L',
    priceEtiqueta: '$9.60/L',
    totalValueEtiqueta: '$0',
    supplier: 'GreenField Chemicals',
    purchaseFecha: 'Apr 5, 2026',
    progress: 0,
    alert:
      'Stock is empty. Add at least 20 L to restore the minimum inventory level.',
    movements: [
      {
        id: 'm1',
        title: 'Stock Added',
        date: 'Apr 5',
        amountEtiqueta: '+40 L',
        kind: 'in',
      },
      {
        id: 'm2',
        title: 'Used — South Field',
        date: 'May 20',
        amountEtiqueta: '−16 L',
        kind: 'out',
      },
      {
        id: 'm3',
        title: 'Used — East Field',
        date: 'Jun 5',
        amountEtiqueta: '−12 L',
        kind: 'out',
      },
      {
        id: 'm4',
        title: 'Used — River Plot',
        date: 'Jun 18',
        amountEtiqueta: '−12 L',
        kind: 'out',
      },
      {
        id: 'm5',
        title: 'Current Balance',
        date: '',
        amountEtiqueta: '0 L',
        kind: 'balance',
      },
    ],
  },
];

export const EQUIPMENT: FarmEquipment[] = [
  {
    id: 'eq-t150',
    name: 'Tractor T-150',
    type: 'Tractor',
    modelAnio: 'Model 2022',
    icon: '🚜',
    status: 'Available',
    hours: 1842,
    hoursEtiqueta: '1,842 hrs',
    serviceIntervalHrs: 250,
    hoursToServicio: 8,
    serviceAlerta: 'Service due in 8 hrs',
    fuelTipo: 'Diesel',
    maintCostEtiqueta: '$3,240',
    serviceProgreso: 0.97,
  },
  {
    id: 'eq-s770',
    name: 'John Deere S770',
    type: 'Harvester',
    modelAnio: 'Model 2021',
    icon: '🌾',
    status: 'Available',
    hours: 920,
    hoursEtiqueta: '920 hrs',
    serviceIntervalHrs: 300,
    hoursToServicio: 80,
    fuelTipo: 'Diesel',
    maintCostEtiqueta: '$1,860',
    serviceProgreso: 0.73,
  },
  {
    id: 'eq-ux4200',
    name: 'Amazone UX 4200',
    type: 'Sprayer',
    modelAnio: 'Model 2023',
    icon: '💧',
    status: 'In Use',
    hours: 640,
    hoursEtiqueta: '640 hrs',
    serviceIntervalHrs: 200,
    hoursToServicio: 45,
    assignedCampo: 'East Field',
    fuelTipo: 'Diesel',
    maintCostEtiqueta: '$920',
    serviceProgreso: 0.78,
  },
];

export const FINANCE_SUMMARY = {
  netProfitEtiqueta: '$33,600',
  incomeEtiqueta: '$41,200',
  expensesEtiqueta: '$7,600',
  costPerHaEtiqueta: '$105',
};

export const EXPENSE_CATEGORIAS: ExpenseCategory[] = [
  {
    id: 'fert',
    label: 'Fertilizer',
    amountEtiqueta: '$2,640',
    progress: 1,
    color: '#5a8bff',
  },
  {
    id: 'fuel',
    label: 'Fuel',
    amountEtiqueta: '$1,890',
    progress: 0.72,
    color: '#f0932b',
  },
  {
    id: 'labor',
    label: 'Labor',
    amountEtiqueta: '$1,450',
    progress: 0.55,
    color: '#a06bff',
  },
  {
    id: 'equip',
    label: 'Equipment',
    amountEtiqueta: '$820',
    progress: 0.31,
    color: '#e24bc0',
  },
];

export const TRANSACTIONS: FarmTransaction[] = [
  {
    id: 'tx-wheat',
    title: 'Winter Wheat Sale',
    subtitle: 'Crop Sales · River Plot · Jul 12',
    amountEtiqueta: '+$41,200',
    kind: 'income',
    icon: '💰',
  },
  {
    id: 'tx-npk',
    title: 'NPK Fertilizer',
    subtitle: 'Fertilizer · North Field · Jul 8',
    amountEtiqueta: '−$2,640',
    kind: 'expense',
    icon: '🧾',
  },
];

export const SEASON_REPORT: SeasonReport = {
  id: 'rpt-2026',
  title: 'Season 2026',
  subtitle: 'Whole-farm season overview · Season 2026',
  metrics: [
    {label: 'Total Fields', value: '4'},
    {label: 'Farmed Area', value: '72.1 ha'},
    {label: 'Completed Tasks', value: '12'},
    {label: 'Total Income', value: '$41,200', tone: 'success'},
    {label: 'Total Expenses', value: '$7,600', tone: 'danger'},
    {label: 'Net Profit', value: '$33,600', tone: 'success'},
    {label: 'Best Field', value: 'River Plot', tone: 'gold'},
  ],
};

export const INVENTORY_CATEGORIAS = [
  'Fertilizers',
  'Sowing',
  'Crop Protection',
  'Fuel',
  'Parts',
] as const;

export const INVENTORY_UNITS = ['kg', 'L', 'bags', 'units'] as const;

export const INCOME_CATEGORIAS = [
  'Crop Sales',
  'Livestock',
  'Subsidies',
  'Other',
] as const;

export const MAINTENANCE_TYPES = [
  'Oil & Filter Service',
  'Tire Service',
  'Engine Repair',
  'General Inspection',
] as const;

export function getInventoryArticulo(id: string) {
  return INVENTORY_ITEMS.find(item => item.id === id);
}

export function getEquipo(id: string) {
  return EQUIPMENT.find(item => item.id === id);
}

export function inventoryTotales(items: InventoryItem[]) {
  const totalValor = items.reduce(
    (sum, item) => sum + item.quantity * item.unitCosto,
    0,
  );
  const needsAtencion = items.filter(
    item => item.status === 'Low Stock' || item.status === 'Out of Stock',
  ).length;
  return {
    totalValueEtiqueta: `$${Math.round(totalValor).toLocaleString('en-US')}`,
    needsAtencion,
  };
}

export function equipmentTotales(items: FarmEquipment[]) {
  return {
    available: items.filter(item => item.status === 'Available').length,
    inUse: items.filter(item => item.status === 'In Use').length,
    service: items.filter(
      item =>
        item.status === 'Service' ||
        (item.hoursToServicio != null && item.hoursToServicio <= 10),
    ).length,
  };
}
